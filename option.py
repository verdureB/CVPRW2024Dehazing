...
def parse_args():
    ...
    data_group.add_argument(
        "--image_size", type=int, default=672, help="训练时裁剪的图像尺寸"
    )
    data_group.add_argument(
    ...
    training_group.add_argument(
        "-wd", "--weight_decay", type=float, default=5e-2, help="权重衰减率"
    )
    training_group.add_argument(
        "-lr", "--learning_rate", type=float, default=1e-4, help="初始学习率"
    )
    training_group.add_argument(
        "-bs", "--batch_size", type=int, default=2, help="批量大小"
    )
    training_group.add_argument(
        "-e", "--epochs", type=int, default=800, help="训练总轮数"
    )
    training_group.add_argument(
        "--pct_start", type=float, default=0.3, help="学习率预热的比例"
    )
    training_group.add_argument(
    ...
    training_group.add_argument(
        "--beta2", type=float, default=0.999, help="AdamW 优化器的 beta2 参数"
    )
    ...
    loss_group.add_argument(
        "--lpips_rate", type=float, default=0.0, help="LPIPS 损失的权重"
    )
    loss_group.add_argument(
        "--gan_g_rate", type=float, default=0.0, help="生成器的 GAN 损失权重"
    )
    loss_group.add_argument(
        "--gan_d_rate", type=float, default=0.0, help="判别器的 GAN 损失权重"
    )
    loss_group.add_argument(
    ...
    experiment_group.add_argument(
        "--exp_name", type=str, default="v1->ema", help="实验名称"
    )
    experiment_group.add_argument(
        "--val_check", type=float, default=1.0, help="验证频率 (多少个 epoch 验证一次)"
    )
    experiment_group.add_argument(
        "--log_step", type=int, default=10, help="日志记录频率 (多少个 batch 记录一次)"
    )
